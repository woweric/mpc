...
%     x0.mup = 0.041+ 0.0025*randn;  % maximum specific growth rate of penicillin [h^-1]
    ...
    rng(Seed_ref+Batch_no +Rand_ref); 
    Rand_ref =Rand_ref+1;
%     x0.S = 1 + randn;      % substrate concentration [g L^{-1}]
    x0.S = 1 ;
    rng(Seed_ref+Batch_no +Rand_ref); 
    Rand_ref =Rand_ref+1;
%     x0.DO2 = 15 + 10*randn  ; % initial dissolved oxygen concentration [mg L^{-1}]
    x0.DO2 = 15;
    rng(Seed_ref+Batch_no +Rand_ref); 
    Rand_ref =Rand_ref+1;
%     x0.X = intial_conds + randn; % initial biomass concentration [g L^{-1}]
    x0.X = intial_conds;
